...
def main():
    website = "caa123_org_sy_qt"

    df = pd.DataFrame(
        columns=['详情页标题', '地区', "x详情页信息", "结束时间", 'x重要提示', '页面网址'])

    url = "https://paimai.caa123.org.cn/pages/lots/list.html?&status=0&attribute=&term=&startTimeStamp=&endTimeStamp=&standardType=19&canLoan=&isRestricted=&insuranceSupport="
    ...
    try:
        ...
        for i in range(5):
            ...
            #                    /html/body/div[3]/div/div[2]/div/ul/li[1]/a/div[2]/div/p[1]/span[2]/span

            for mov1 in datas:
                # 进入详情页点击对象
                title_obj = mov1.ele('x:.//a')

                # 获取列表页数据
                time_text = mov1.ele('x:.//a/div[2]/div/p[1]/span[2]/span').text
